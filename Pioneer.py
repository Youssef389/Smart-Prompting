...
model_name = contentdriveMyDrivefalcon-rw-1b
...
def analyze_example(tweet, label=None)
    prompt = fYou are a sentiment classifier. Think step-by-step before giving a final answer.nn

    for ex in few_shot_examples
        prompt += fTweet {ex['tweet']}n
        prompt += fReasoning The tweet expresses {ex['label']} sentiment.n
        prompt += fSentiment {ex['label']}nn

    prompt += fTweet {tweet}nReasoning
...
print(=== Selected Examples ===n)
for ex in sorted_examples
    print(fTweet {ex['tweet']})
    print(fLabel {ex['label']})
    print(fReasoning {ex['reasoning'].splitlines()[0]})
    print(fScore {ex['final_score'].4f})
    print(-  50)

print(n=== Evaluation ===)
print(fTweet {new_example['tweet']})
print(fReasoning {results[-1]['reasoning']})
print(fPredicted Sentiment {results[-1]['reasoning'].split('Sentiment')[-1].strip()})
print(fFinal Score {results[-1]['final_score'].4f})
